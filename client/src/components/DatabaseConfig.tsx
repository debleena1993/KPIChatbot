  ...
  if (isLoading) {
    return (
      <Card>
        ...
        <CardContent>
          <div className="text-center text-muted-foreground">Loading configurations...</div>
        </CardContent>
      </Card>
    );
  }
      ...
      <Card>
        ...
        <CardContent>
          <div className="text-center text-muted-foreground">
            No database connections configured. Connect to a database to get started.
          </div>
        </CardContent>
      </Card>
    );
          ...
          {config.connections.map((connection) => (
            <div
              ...
              className={`p-4 border rounded-lg ${
                connection.isActive ? 'border-blue-500 bg-blue-50' : 'border-gray-200'
              }`}
            >
              <div className="flex items-center justify-between">
                <div className="flex-1">
                  ...
                  <div className="grid grid-cols-2 gap-2 text-sm text-muted-foreground">
                    ...
                    <div>Last Connected: {formatDate(connection.lastConnected)}</div>
                    {connection.schema && (
                      <div>Tables: {connection.schema.totalTables || Object.keys(connection.schema.tables || {}).length}</div>
                    )}
                  </div>
                </div>
            ...
            {selectedSchema && (
              <div className="space-y-4">
                <div className="text-sm text-muted-foreground mb-4">
                  Schema extracted on: {selectedSchema.extractedAt ? new Date(selectedSchema.extractedAt).toLocaleString() : 'Unknown'}
                </div>
                
                {Object.entries(selectedSchema.tables || {}).map(([tableName, table]: [string, any]) => (
                  <Card key={tableName}>
